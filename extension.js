...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('extension.insertText', insertText);
    context.subscriptions.push(disposable);

    // Registro del comando para insertar texto check
    let disposableDos = vscode.commands.registerCommand('extension.insertSquare', insertSquare);
    context.subscriptions.push(disposableDos);

    // Registro del comando para insertar fecha
    let disposableTres = vscode.commands.registerCommand('extension.insertCurrentDate', insertCurrentDate);
...
function updateDecorations() {
    ...
    while (match = temaRegEx.exec(text)) {
        ...
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'Tema ' };
    ...
    while (match = nuevoTextoRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'Check' };
    ...
    while (match = arrobaRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'arroba' };
    ...
    while (match = validarRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'validar' };
    ...
    activeEditor.setDecorations(validarDecoration, validarDecorations);
     ...
     while (match = checkDosRegEx.exec(text)) {
         const startPos = activeEditor.document.positionAt(match.index);
         const endPos = activeEditor.document.positionAt(match.index + match[0].length);
         const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'checkDos' };
     ...
     while (match = fechaRegEx.exec(text)) {
         const startPos = activeEditor.document.positionAt(match.index);
         const endPos = activeEditor.document.positionAt(match.index + match[0].length);
         const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'fecha' };
    ...
    while (match = altaRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'alta' };
    ...
    while (match = mediaRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'media' };
    ...
    const comentarioUnoRegEx = /\/\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\//g;
    while (match = comentarioUnoRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'comentarioUno' };
        comentarioUnoDecorations.push(decoration);
    }
    activeEditor.setDecorations(comentarioUnoDecoration, comentarioUnoDecorations);
    ...
    while (match = comentarioDosRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'comentarioDos' };
        comentarioDosDecorations.push(decoration);
    }
    activeEditor.setDecorations(comentarioDosDecoration, comentarioDosDecorations);
    ...
    while (match = comentarioTresRegEx.exec(text)) {
        const startPos = activeEditor.document.positionAt(match.index);
        const endPos = activeEditor.document.positionAt(match.index + match[0].length);
        const decoration = { range: new vscode.Range(startPos, endPos), hoverMessage: 'comentarioTres' };
        comentarioTresDecorations.push(decoration);
    }
    activeEditor.setDecorations(comentarioTresDecoration, comentarioTresDecorations);
...
function insertText() {
    ...
    const insertPosition = (nonWhiteSpaceIndex !== -1) 
